...
*Architecture Decision Records deserve to be more than files.*
...
What survives is a partially-truthful fossil record: markdown files in a `doc/adr/` folder, if the team was disciplined; scattered comments and commit messages, if they were not. The knowledge that didn't make it into prose — the road not taken, the trade-off reasoning, the assumption that seemed too obvious to state — evaporates the moment the meeting ends.

The next generation of developers inherits the decision without the deliberation. Chesterton's Fence applies in full: do not remove a fence until you know why it was built. But you cannot know why a fence was built if the builder left no record of the wolves. The developers who follow learn *what* was decided but not *why*, and so they cannot know when the reasons have changed. They tear down fences that were load-bearing. They repeat mistakes that were already made, relitigate questions that were already settled, and break constraints they didn't know existed.
...
ADR-O is a formal ontology for Architecture Decision Records. Its scope is deliberately narrow: a single decision, its context, its alternatives, its rationale, and its typed relationships to every other decision in the graph. That is the atom. ADR-O does not try to model all architectural knowledge — only the unit from which all architectural knowledge can be composed.
...
And the graph changes everything. It can answer questions a folder of markdown files cannot: which decisions constrain this component? Which past choices conflict with this proposed change? What was the reasoning that led the team away from the approach now being reconsidered? What is the full supersession chain from the original design to what runs in production today? These are not keyword searches against prose — they are traversals of an explicit, typed graph, returning consistent answers every time.
