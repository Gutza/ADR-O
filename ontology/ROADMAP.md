...
The single most important finding from that exercise is the one that reframes the entire roadmap: **ADR-O 0.2.x is a high-performance Decision Index – it is not yet a Knowledge System.** It answers "what was decided and why" as a set of weighted claims with reliable IRI identity. With 0.2.3, social-role attribution (`authoredBy` / `decidedBy` / `consulted` / `informed`) became first-class. With 0.2.5, ADR-0025 added an **ADL scope** causal topology (`constrainedBy`, `prohibitedBy`, `recommends`, `discourages`, `enabledBy`) and formally adopted ZIO's **Y-statement** as the design contract for the **ADR scope**: the six clauses of a Y-statement map directly onto ADR-O's ADR scope constructs, and the **ADL scope** "Therefore" link is now machine-traversable with RFC 2119 modal strength. What the model still cannot do is answer three Y-statement questions losslessly on roundtrip: *"to achieve what?"* (no `Goal` class, no teleological predicate — `OutcomeFact`+`Benefit` is retrospective, not intentional), *"why not the neglected option?"* (no `Criterion` class, no structured evaluation — comparative rationale collapses into prose), and *"how do the premises inside one record connect to the conclusion?"* (no ADR scope `Consideration→Consideration` inference edges — ADR scope syllogisms are invisible to SPARQL). Closing those three remaining gaps is what the horizons below are organized around.
...
- **Partial Argumentative Flatness:** **ADL scope** "Therefore" links shipped with ADR-0025 (`constrainedBy` / `prohibitedBy` / `recommends` / `discourages` / `enabledBy`, RFC 2119 modal strength). What remains invisible to SPARQL is the *ADR scope* dimension: presupposition or entailment relationships between `Consideration` nodes *within one record's deliberation* — the Y-statement's `Facing → Decided` bridge — have no inference-edge representation.
...
Alignment status between `adr-o.ttl` and each governing ADR, updated to reflect 0.2.4-draft:
...
- **ADR-0006–ADR-0026 (ontology vocabulary and shape, post-factum from design notes):** ✅ ADL-level direction is aligned through ADR-0026. The `.ttl` baseline is now 0.2.5-draft and includes unified amendment semantics from ADR-0019/ADR-0020 (`amends`/`amendedBy`, no separate clarification predicate, no `Amended` status), social-role predicates from ADR-0021 (`authoredBy`, `decidedBy`, `consulted`, `informed`), coexistence semantics from ADR-0022 (`dcterms:creator` and `adr-o:authoredBy`), record-level typing from ADR-0023 (`adr-o:hasType`), sequential-ID policy alignment from ADR-0024, causal topology predicates from ADR-0025 (`constrainedBy`, `prohibitedBy`, `recommends`, `discourages`, `enabledBy`, `materializes`, `justifiedBy`) with the Y-statement formally adopted as the **ADR scope** design contract and the **ADL scope** causal predicates with RFC 2119 modal strength, and ontology provenance linkage from ADR-0026 (`adr-odr` namespace and `justifiedBy` annotations). See individual files under `ADL/`.
...
- Introduce a property (tentatively `adr-o:presupposes` or `adr-o:entails`) between `Consideration` nodes, scoped to ADR scope use. This is the ADR scope counterpart of the ADL scope causal predicates already in 0.2.5-draft.
- Scope strictly to `Consideration → Consideration` edges within one record's `hasContext` or `hasDeliberation` list. Do not extend to ADL scope use (that is already covered by ADR-0025's ADL scope predicates).
- Design prerequisite: a dedicated ADL entry that resolves whether the direction is `Consideration → Consideration` (presupposition) or `Consideration ← Consideration` (entailment), and whether inference edges should also be permitted between `ContextFact`s and a `chosenAlternative` node (the `Facing → Decided` bridge proper).
